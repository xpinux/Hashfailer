...
def crack_hash(tool, hash_input, mode, dictionary):
    possible_hash_types = identify_hash(hash_input)
    
    if not possible_hash_types:
        print(colored("No matching hash type found.", "red"))
        return
    
    # Ask the user for their preference
    print(colored("Do you want to crack every possible hash type? (yes/no): ", "cyan"))
    user_choice = input().lower().strip()
    
    if user_choice == "yes":
        hash_types_to_crack = possible_hash_types
    elif user_choice == "no":
        # Select the most probable hash type
        most_probable_hash_type = max(possible_hash_types, key=lambda x: x[3])
        hash_types_to_crack = [most_probable_hash_type]
    else:
        print(colored("Invalid choice. Cracking the most probable hash type.", "yellow"))
        most_probable_hash_type = max(possible_hash_types, key=lambda x: x[3])
        hash_types_to_crack = [most_probable_hash_type]
    
    for hash_type in hash_types_to_crack:
        print(colored(f"Attempting to crack as {hash_type[0]} hash using {tool}...", "yellow"))
        
        if tool == "hashcat":
            cmd = ["hashcat", "-m", hash_type[1], "-a", "0", hash_input, dictionary]
        elif tool == "john":
            with tempfile.NamedTemporaryFile(mode="w", delete=False) as temp_file:
                temp_file.write(hash_input)
                temp_file_path = temp_file.name

            cmd = ["john", "--format=" + hash_type[2], "--wordlist=" + dictionary, temp_file_path]
...
def identify_hash(hash_str, threshold=0.7):
    matches = []
    for hash_type in HASH_TYPES:
        regexes = hash_type.get("regexes", [hash_type["regex"]])
        for regex in regexes:
            match = re.match(regex, hash_str)
            if match and match.span()[1] / len(hash_str) >= threshold:
    ...
    # Assign higher probability to the most likely hash type and distribute the rest
    most_likely_probability = 70.0
    remaining_probability = (100.0 - most_likely_probability) / (total_matches - 1)

    results = []
    for i, match in enumerate(matches):
        probability = most_likely_probability if i == 0 else remaining_probability
